...
Notificaciones (recordatorios push/email):
`NEXT_PUBLIC_FIREBASE_VAPID_KEY` (Firebase Console → Cloud Messaging → Web Push certificates), `FIREBASE_SERVICE_ACCOUNT_KEY` (JSON, ya usado por firebase-admin), `CRON_SECRET` (bearer para `/api/cron/*`; Vercel lo inyecta auto en jobs declarados en `apps/dashboard/vercel.json`)
